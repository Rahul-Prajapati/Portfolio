...
const Coursesection = () => {
    ...
    const onFinish = async (values) => {
        try {
            ...
            if (selectedEdit) {
                ...
                response = await fetch("/api/portfolio/update-course", {
                    method: 'POST',
                    mode: 'no-cors',
                    headers: {
                ...
                response = await fetch("/api/portfolio/add-course", {
                    method: 'POST',
                    mode: 'no-cors',
                    headers: {
                        'Content-Type': 'application/json',
    ...
    const onDelete = async (item) => {
        try {
            ...
            const response = await fetch("/api/portfolio/delete-course", {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    "Access-Control-Allow-Origin": "*",
    ...
    return (
        <div>
            ...
            {(type === "Add" || selectedEdit) && (
                <Modal
                    open={showEditOption}
                    title={selectedEdit ? "Edit course" : "Add course"}
                    footer={null}
                    onCancel={
                        () => {
                            setShowEditOption(false)
